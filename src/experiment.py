...
class RetinopathyDatasetTrain(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if '.jpg' not in img_name:
            img_name = img_name + self.images_format
    ...
    def _postprocess_model_for_stage(self, stage: str, model: nn.Module):
        ...
        if stage in ["stage1"]:
            model_.load_state_dict(
                torch.load('logs/efficientnet-4/pretraining_combined_256/checkpoints/stage2.18.pth')['model_state_dict']
            )
        ...
        if stage in ["stage2"]:
            #model_.load_state_dict(
            #    torch.load('logs/efficientnet-3/pretraining_combined_256/checkpoints/stage1.7.pth')['model_state_dict']
            #)
            for param in model_.model.parameters():
                param.requires_grad = True
            print('Learnable parameters: ', sum(p.numel() for p in model_.model.parameters() if p.requires_grad))
